...
class FPNSSDBoxCoder:
    ...
    def _get_default_boxes_new(self):
        fm_sizes = ((180, 320), (90, 160), (45, 80), (23, 40), (12, 20), (6, 10), (3, 5), (2, 3))
        # fm_sizes = self.model.fm_sizes_adapted
        boxes = []
    ...
    def decode(self, loc_preds, cls_preds, score_thresh=0.6, nms_thresh=0.45):
        ...
        '''
        default_boxes_new = self._get_default_boxes_new()
        variances = (0.1, 0.2)
        xy = loc_preds[:,:2] * variances[0] * default_boxes_new[:,2:] + default_boxes_new[:,:2]
        wh = torch.exp(loc_preds[:,2:]*variances[1]) * default_boxes_new[:,2:]
        # xy = loc_preds[:,:2] * variances[0] * self.default_boxes_new[:,2:] + self.default_boxes_new[:,:2]
        # wh = torch.exp(loc_preds[:,2:]*variances[1]) * self.default_boxes_new[:,2:]
        ...
        try:
            ...
            keep = box_nms(boxes, scores, 0.5)
            boxes_final.append(boxes[keep])
            labels_final.append(labels[keep])
            scores_final.append(scores[keep])

            boxes = torch.cat(boxes_final, 0)
            labels = torch.cat(labels_final, 0)
            scores = torch.cat(scores_final, 0)
